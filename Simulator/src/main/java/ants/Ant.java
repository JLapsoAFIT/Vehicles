...
public class Ant extends Vehicle {
    ...
    public Ant(World<SimulationBody> myWorld) {
        ...
        int max = 15;
        int min = -15;
        double xPos = Math.floor(Math.random()*(max-min+1)+min); // done just for code clarity
        double yPos = Math.floor(Math.random()*(max-min+1)+min);
        this.translate(xPos,yPos);

        home = new Vector2(xPos,yPos); // center of the screen for now.
     ...
     * @param resources
     */
    public void decide(ArrayList<Ant> allAgentAnts, ArrayList<Resource> resources) {
        action.clear();
        deathAndTaxes();
     ...
     *
     */
    private void updateAnts(ArrayList<Ant> allAgentAnts) {
        for(Ant antObj : allAgentAnts) {
